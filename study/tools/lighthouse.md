...
# Lighthouse  를 이용한 문제 정의 및 랜더링 속도 개선
...
| 지표                                 | 설명                                                | 기준                                                |
| ---------------------------------- | ------------------------------------------------- | ------------------------------------------------- |
| **FCP (First Contentful Paint)**   | 사용자가 페이지로 이동한 후 브라우저에서 첫번째 DOM 콘텐츠를 렌더링하는데 걸리는 시간 | <p>0-1.8s: 좋음<br>1.8-3.0s: 개선 필요<br>3.0s+: 나쁨</p> |
| **LCP (Largest Contentful Paint)** | 표시 영역에 표시되는 가장 큰 이미지 또는 텍스트 블록 렌더링 시간             | <p>0-2.5s: 좋음<br>2.5-4.0s: 개선 필요<br>4.0s+: 나쁨</p> |
| **SI (Speed Index)**               | 페이지 로드 중 콘텐츠가 시각적으로 표시되는 속도를 측정                   | <p>0-3.4s: 좋음<br>3.4-5.8s: 개선 필요<br>5.8s+: 나쁨</p> |
| **TBT (Total Blocking Time)**      | 사용자 입력에 응답하지 못하도록 차단된 시간                          | <p>0-200ms: 좋음<br>200-600ms: 개선 필요600ms+: 나쁨</p>  |
