...
```markdown
자바 java 파일을 바이트 코드로 변환하고 그걸 클래스 로더가 데이터 영역에 로드하고 실행엔진이 자바 코드를 실행한다. 

단순히 자바 코드를 바꾸는게 아니라 자바 컴파일 을 하여 바이트 코드로 전환 해야한다는 점이다. 

근본적으로 jvm 바이트 코드 발사대 (실행기) 이다. 

바이트 코드를 런타임 영역에 로드하고 실행엔진이 바이트 코드를 실행한다. 

클래스 로더의 특징은 다음과 같다
계층 구조 :: 클래스 로더는 부모 자식 같은 계층 구조를 이룬다. 최상위 클래스 로더는 부트스트랩 로더이다 
```
